/**
 * Browser Automation Widget — Two-Layer Architecture
 *
 * Layer 1 (this file): Cheap DOM parser + action executor (runs in browser)
 * Layer 2 (server): LLM planner (API call)
 *
 * Loop: extract → plan → act → observe → extract again → repeat
 */
export function getWidgetScript(): string {
  ...
  var msgCount = 0;
  var loopCount = 0;
  var MAX_LOOPS = 10; // BUG FIX #4: Was 20
  ...
  // Page overlay for agent activity
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // WAIT FOR CHANGE — MutationObserver-based stability detection
  // BUG FIX #3: Single cleanup() function, called on stable, timeout, AND stopped
  // ═══════════════════════════════════════════════════════════════════════════════

  function waitForChange(minStable, maxWait, expectNavigation) {
    ...
    return new Promise(function(resolve) {
      ...
      // BUG FIX #3: Single cleanup function
      function cleanup() {
        if (resolved) return;
        resolved = true;
      ...
      function check() {
        ...
        // URL changed = navigation, reset timer
        ...
        if (stable || timeout) {
          cleanup();
          console.log('[agent] waitForChange:', stable ? 'stable' : 'timeout',
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // BUILD SELECTOR — Creates reliable selector for element
  // ═══════════════════════════════════════════════════════════════════════════════
  ...
  function buildSelector(el) {
    ...
    if (placeholder) {
      ...
      for (var j = 0; j < inputs.length; j++) {
        if (inputs[j].getAttribute('placeholder') === placeholder) phCount++;
      }
      if (phCount === 1) return '[placeholder="' + placeholder + '"]';
    }

    // 5. Unique aria-label
    var ariaLabel = el.getAttribute('aria-label');
    if (ariaLabel) {
      var allWithLabel = document.querySelectorAll('[aria-label="' + ariaLabel + '"]');
      if (allWithLabel.length === 1) return '[aria-label="' + ariaLabel + '"]';
    }

    // 6. Data attribute marker (fallback)
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // LAYER 1: DOM DISTILLATION — Semantic DOM extraction
  // Walks TEXT NODES directly, captures only interactive + semantic elements
  // ═══════════════════════════════════════════════════════════════════════════════
  ...
  function distillDOM() {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 1: Visible text (headings, alerts, content)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    while ((textNode = textWalker.nextNode())) {
      ...
      // Skip text inside interactive elements (captured separately)
      ...
      if (parentTag === 'H1') lines.push('#1"' + text.slice(0, 80) + '"');
        ...
        lines.push('!ok"' + text.slice(0, 80) + '"');
      }
      else if (parent.classList.contains('success') || parent.classList.contains('toast')) {
        lines.push('!ok"' + text.slice(0, 80) + '"');
      }
      else if (parent.classList.contains('error') || parent.classList.contains('danger')) {
        lines.push('!err"' + text.slice(0, 80) + '"');
      }
      else {
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // ACTION EXECUTORS — doAction(type, target, value)
  // ═══════════════════════════════════════════════════════════════════════════════
  ...
  function clickElement(idx) {
    ...
    if (!el) {
      ...
      var hint = available ? 'Available: ' + available + '. DOM may have changed.' : 'No elements indexed.';
  ...
  var stopBtn = shadow.getElementById('stop');
  ...
  function setLoading(on) {
    ...
    if (on) { stopBtn.classList.add('show'); stopped = false; }
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // ACTION EXECUTION — executeActions with duplicate detection
  // ═══════════════════════════════════════════════════════════════════════════════
  ...
  function executeActions(actions, onComplete) {
    ...
    function next() {
      ...
      if (window.location.href !== startUrl) {
        ...
        hideOverlay();
        waitForChange(400, 2000).then(function() { onComplete(results); });
        return;
      }
      if (idx >= filtered.length) {
        hideOverlay();
        waitForChange(300, 1500).then(function() { onComplete(results); });
        return;
      }

      var action = filtered[idx++];

      // BUG FIX #2: Ensure description exists
      if (!action.description) {
        action.description = action.type + ' element ' + (action.index || 'page');
      }

      var pill = showActionPill(action.description);
      ...
      setTimeout(function () {
        ...
        waitForChange(mightNavigate ? 500 : 250, mightNavigate ? 4000 : 2000, mightNavigate).then(next);
      }, 60);
    }
    next();
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // AGENTIC LOOP — extract → plan → act → observe → repeat
  // ═══════════════════════════════════════════════════════════════════════════════
  ...
  function handleResponse(data) {
    ...
    if (actions.length > 0) {
      executeActions(actions, function (actionResults) {
        ...
        else           { sendObservation(actionResults); }
      });
    } else {
      if (data.done) {
        setLoading(false);
        ...
        // Agent said something but no actions — might need another turn
        sendObservation([]);
      } else {
  ...
  function sendObservation(actionResults) {
    if (stopped) return;

    // BUG FIX #4: MAX_LOOPS = 10
    ...
    // Wait for DOM to stabilize before capturing
    waitForChange(300, 2000, false).then(function() {
      if (stopped) return;
