  ...
  function executeActions(actions, onComplete) {
    ...
    function next() {
      ...
      if (window.location.href !== startUrl) {
        ...
        hideOverlay();
        setTimeout(function() { onComplete(results); }, 300);
        return;
      }
      if (idx >= actions.length) {
        hideOverlay();
        setTimeout(function() { onComplete(results); }, 800);
        return;
      }
      ...
      setTimeout(function () {
        ...
        try {
          if (action.type === 'click') {
            ...
            // Wait action — pause for page to settle after navigation/modals
            ...
            return; // Skip the default setTimeout(next, 700) below
          }
          else if (action.type === 'read') {
            // Read action — get full text content of an element (for truncated content)
            var readResult = readElement(action.selector);
            success = readResult.success;
            error = readResult.error;
            // Store the text in the result so it goes back to the agent
            results.push({
              type: 'read',
            ...
            setTimeout(next, 100); // Quick continuation for read
            return;
          }
        } catch (e) {
