...
import { runAgent, type LLMMessage, type AgentAction, type Subtask } from "../lib/agent";
...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
      ...
      actionResults && actionResults.length > 0
        ? actionResults
            .map((r) => {
              ...
              // Include text for read actions (truncated if very long)
              const textResult = r.type === "read" && r.text
                ? ` → "${r.text.slice(0, 500)}${r.text.length > 500 ? "..." : ""}"`
                : "";
              return `${icon}${r.type}${target}${err}${textResult}`;
            })
            .join(" | ")
        : "no actions";
    ...
    // 6c. Extract previous subtasks and recent actions for guardrails ─────────
    ...
    for (const row of (historyRows ?? []).slice().reverse()) {
      if (row.role === "assistant") {
        try {
          ...
          // Get subtasks from the most recent assistant turn
          if (previousSubtasks.length === 0 && parsed.subtasks) {
            previousSubtasks = parsed.subtasks;
          }
          // Collect recent actions (last ~10 actions across turns)
          if (parsed.actions && recentActions.length < 10) {
            for (const a of parsed.actions) {
              if (recentActions.length < 10) {
                recentActions.push(`${a.type}:${a.selector || ""}`);
              }
            }
          }
        } catch {
    ...
    console.log("[chat] GOAL:", (originalGoal || message || "").slice(0, 80));
    console.log("[chat] DOM:", Array.isArray(dom) ? dom.length : dom?.elements?.length, "elements");
    if (isActionResult && actionResults?.length) {
      const resultSummary = actionResults.map(r => `${r.success ? "✓" : "✗"}${r.type}`).join(" ");
      console.log("[chat] LAST ACTIONS:", resultSummary);
    }
    console.log("[chat] HISTORY:", recentActions.length, "recent actions,", previousSubtasks.length, "subtasks");
    console.log("[chat] ───────────────────────────────────────────────────────");
