...
| read | Get full content of truncated text elements (see TRUNCATED CONTENT) | {"type":"read","selector":"[data-ag-ref=\\"_agref_5\\"]","description":"Read full API key"} |
...
Long text elements in the DOM are truncated to ~100 characters with "..." at the end.
Truncated elements include:
- \`truncated: true\` — indicates content was cut off
- \`fullLength: 523\` — original character count
- \`selector: "[data-ag-ref=\\"_agref_5\\"]"\` — stable selector to read full content

**To get full content, use the read action:**
\`{"type":"read","selector":"[data-ag-ref=\\"_agref_5\\"]","description":"Read full script tag"}\`

The result will include the full text in the action results.

## PAGE CHANGE DETECTION

Observations include page context with navigation info:
- \`page.url\` — current page URL
- \`page.urlChanged: true\` — indicates the page navigated since last observation
- \`page.previousUrl\` — the URL before navigation (only when urlChanged is true)

**When you see urlChanged: true:**
- The page has navigated — your previous actions caused a redirect or the user navigated
- You do NOT need to click "go back" or "return to dashboard" — the navigation already happened
- Update your subtasks to reflect the new page state
- Continue from the current page, don't try to undo automatic redirects
...
export interface AgentAction {
  type: "click" | "type" | "scroll" | "hover" | "wait" | "read";
  selector?: string;
  text?: string;
  direction?: "up" | "down";
  container?: string;
...
export interface ActionResult {
  ...
  success: boolean;
  error?: string;
  text?: string; // For read action results
}
...
// ═══════════════════════════════════════════════════════════════════════════
// GUARDRAILS — Don't trust the LLM blindly
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Checks if a selector targets a "dangerous" element that might cause navigation,
 * form submission, or modal opening. These should NEVER be batched with other actions.
 */
function isDangerousSelector(selector?: string, description?: string): boolean {
  if (!selector && !description) return false;
  const text = ((selector || "") + " " + (description || "")).toLowerCase();

  // Submit/save actions
  if (text.includes("submit") || text.includes("save")) return true;
  // Navigation
  if (text.includes("next") || text.includes("continue") || text.includes("proceed")) return true;
  // Confirmation dialogs
  if (text.includes("confirm") || text.includes("ok") || text.includes("yes")) return true;
  // Destructive actions
  if (text.includes("delete") || text.includes("remove") || text.includes("cancel")) return true;
  // Auth actions
  if (text.includes("sign") || text.includes("log") || text.includes("register")) return true;
  // Payment/checkout
  if (text.includes("pay") || text.includes("checkout") || text.includes("purchase") || text.includes("buy")) return true;
  // Add to cart (might redirect)
  if (text.includes("add to cart") || text.includes("add to bag")) return true;

  return false;
}

/**
 * Validates and fixes agent response before execution.
 * Catches common LLM mistakes that cause loops and failures.
 */
function validateAndFixResponse(
  ...
  // 1. ENFORCE SINGLE-ACTION for dangerous buttons
  // If there's a dangerous action (submit, next, delete) mixed with safe actions,
  // keep the SAFE actions and defer the dangerous one to next turn
  const dangerousIdx = response.actions.findIndex(a =>
    a.type === "click" && isDangerousSelector(a.selector, a.description)
  );
  if (dangerousIdx !== -1 && response.actions.length > 1) {
    // Keep actions BEFORE the dangerous one (they're safe to batch)
    const safeActions = response.actions.slice(0, dangerousIdx);
    if (safeActions.length > 0) {
      // There are safe actions to do first - defer the dangerous click
      response.actions = safeActions;
      guardrailsApplied.push("deferred_dangerous_action");
      console.log("[guardrail] deferred dangerous action, keeping", safeActions.length, "safe actions first");
    } else {
      // Dangerous action is first - just keep it alone
      response.actions = [response.actions[dangerousIdx]];
      guardrailsApplied.push("isolated_dangerous_action");
      console.log("[guardrail] isolated dangerous action:", response.actions[0].description);
    }
  }

  // 2. PRESERVE SUBTASKS if agent forgot to include them
  if (response.subtasks.length === 0 && previousSubtasks.length > 0) {
    ...
    console.log("[guardrail] preserved previous subtasks");
  }

  // 3. BLOCK DUPLICATE ACTIONS (same action attempted recently)
  const originalCount = response.actions.length;
  response.actions = response.actions.filter(a => {
    const key = `${a.type}:${a.selector || ""}`;
    const recentCount = recentActions.filter(r => r === key).length;
    if (recentCount >= 2) {
      console.log("[guardrail] blocked duplicate action:", key, `(attempted ${recentCount}x)`);
      return false;
    }
    return true;
  });
  ...
  // 4. DETECT STALL - no actions, no message, not done
  if (response.actions.length === 0 && !response.message && !response.done) {
    response.done = true;
    response.message = "I wasn't able to complete this task. Please try rephrasing or breaking it into smaller steps.";
    guardrailsApplied.push("forced_done_on_stall");
    console.log("[guardrail] forced done on stall");
  }

  // 5. CAP ACTIONS PER TURN (prevent runaway batching)
  if (response.actions.length > 5) {
    response.actions = response.actions.slice(0, 5);
    guardrailsApplied.push("capped_actions");
    console.log("[guardrail] capped actions to 5");
  }
...
export async function runAgent(params: {
  ...
  const {
    history, userMessage, originalGoal, dom, isActionResult, actionResults, pastActions,
    previousSubtasks = [], recentActions = []
  } = params;
  ...
  if (isActionResult) {
    ...
    if (pageContext) {
      // Highlight page changes prominently so agent knows navigation occurred
      if (pageContext.urlChanged) {
        currentContent += `**⚠️ PAGE CHANGED:** ${pageContext.previousUrl} → ${pageContext.url}\n`;
        currentContent += `(Navigation happened automatically — do NOT try to undo this redirect)\n\n`;
      } else {
        currentContent += `**PAGE:** ${pageContext.url}\n`;
      }
      currentContent += `**TITLE:** ${pageContext.title}\n\n`;
    }
    ...
    if (actionResults && actionResults.length > 0) {
      ...
      for (const result of actionResults) {
        ...
        // Include text content for read actions
        if (result.type === "read" && result.text) {
          const displayText = result.text.length > 800
            ? result.text.slice(0, 800) + "... (truncated)"
            : result.text;
          currentContent += `   → Content: "${displayText}"\n`;
        }
      }
      currentContent += "\n";
    }
  ...
  console.log("\n[agent] ═══════════════════════════════════════════════════════");
  console.log("[agent] TOKENS:", `${tokens.prompt} prompt + ${tokens.completion} completion = ${tokens.total} total`);
  console.log("[agent] THINKING:", raw.slice(0, 200).replace(/\n/g, " ") + (raw.length > 200 ? "..." : ""));
  ...
  // Structured summary
  const actionSummary = Array.isArray(parsed.actions)
    ? (parsed.actions as Array<{type?: string; selector?: string}>).map(a => `${a.type}(${a.selector || ''})`).join(", ")
    : "none";
  console.log("[agent] ACTIONS:", actionSummary || "none");
  console.log("[agent] SUBTASKS:", Array.isArray(parsed.subtasks) ? parsed.subtasks.length : 0);
  console.log("[agent] DONE:", parsed.done ? "YES" : "no");
  console.log("[agent] ═══════════════════════════════════════════════════════\n");
  ...
  const validated = validateAndFixResponse(rawResult, previousSubtasks, recentActions);

  if (validated.guardrailsApplied.length > 0) {
    console.log("[agent] guardrails applied:", validated.guardrailsApplied.join(", "));
  }
