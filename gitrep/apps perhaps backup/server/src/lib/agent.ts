...
{"type":"click","index":5,"description":"Click Add to Cart"} — click element
{"type":"type","index":2,"text":"hello@email.com","description":"Enter email"} — type in input
{"type":"hover","index":3,"description":"Hover menu"} — hover (for dropdowns)
{"type":"scroll","direction":"down","description":"Scroll page down"} — scroll page
{"type":"scroll","index":8,"description":"Scroll to element"} — scroll element into view
{"type":"scroll","container":5,"direction":"down"} — scroll within container
{"type":"wait","ms":1000} — wait

**IMPORTANT:** Always include "description" for click actions. Use "filter" or "checkbox" in description when clicking filters/toggles (these can be batched).
...
function validateResponse(
  ...
  // 2. Enforce ONE NAVIGATION CLICK per turn (but allow multiple filter/toggle clicks)
  // Links and navigation buttons = one per turn (page may change)
  // Checkboxes, filters, toggles = OK to batch (same page)
  const clickActions = response.actions.filter((a) => a.type === "click");
  if (clickActions.length > 1) {
    // Check if these look like navigation clicks or safe toggles
    // Safe patterns: descriptions mentioning filter, checkbox, toggle, select, option
    const safePatterns = /filter|checkbox|toggle|select|option|check|uncheck|enable|disable/i;
    const allSafe = clickActions.every((a) => {
      const desc = a.description || "";
      return safePatterns.test(desc);
    });
