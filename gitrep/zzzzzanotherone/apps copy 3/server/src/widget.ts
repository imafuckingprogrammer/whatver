...
export function getWidgetScript(): string {
  ...
  function distillDOM() {
    ...
    var textEls = document.querySelectorAll('p, li, span, label, td, th, dt, dd, figcaption');
    for (var i = 0; i < textEls.length && items.length < 180; i++) {
      ...
      // Only capture meaningful text (not too short, not too long)
  ...
  window.__agentActions = { clickElement: clickElement, typeInElement: typeInElement,
    ...
    scrollInContainer: scrollInContainer, distillDOM: distillDOM };
