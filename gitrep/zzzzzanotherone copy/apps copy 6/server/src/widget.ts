...
export function getWidgetScript(): string {
  ...
  function distillDOM() {
    ...
    while ((textNode = walker.nextNode())) {
      ...
      if (parentTag === 'H1') lines.push('#1"' + text.slice(0, 60) + '"');
      else if (parentTag === 'H2') lines.push('#2"' + text.slice(0, 50) + '"');
      else if (parentTag === 'H3') lines.push('#3"' + text.slice(0, 40) + '"');
      else if (parent.getAttribute('role') === 'alert' || parent.closest('[role="alert"]')) {
        lines.push('!alert"' + text.slice(0, 60) + '"');
      } else if (parent.classList.contains('error') || parent.closest('.error, [class*="error"]')) {
        lines.push('!err"' + text.slice(0, 60) + '"');
      } else if (parent.classList.contains('success') || parent.closest('.success, [class*="success"]')) {
        lines.push('!ok"' + text.slice(0, 60) + '"');
      } else {
    ...
    for (var i = 0; i < interactives.length && index <= 80; i++) {
      ...
      if (elText) desc += '"' + elText + '"';
        ...
        if (path && path.length < 20) desc += '(->' + path + ')';
      }
      ...
      if (el.validationMessage && !el.checkValidity()) {
        desc += ' !' + el.validationMessage.slice(0, 20);
      }
  ...
  // Debug: copy full DOM payload to clipboard
  window.__agentDebug = function() {
    var dom = distillDOM();
    var payload = JSON.stringify(dom, null, 2);
    navigator.clipboard.writeText(payload).then(function() {
      console.log('[agent] DOM copied to clipboard (' + dom.elements.length + ' elements)');
      console.log(payload);
    });
    return dom;
  };
  ...
  document.addEventListener('keydown', function(e) {
    ...
    // Ctrl+Shift+D = debug DOM
    if (e.ctrlKey && e.shiftKey && e.key === 'D') {
      e.preventDefault();
      window.__agentDebug();
      alert('DOM copied to clipboard + logged to console');
    }
  });
