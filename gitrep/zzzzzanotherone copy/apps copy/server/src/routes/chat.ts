...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cap at last 10 messages to prevent context overflow
    const recentRows = (historyRows ?? []).slice(-10);
    ...
    // DISABLED: Was bloating the system prompt with huge step data
    ...
    //   .limit(3);
    const pastActions: Array<{ task_description: string; steps: unknown }> = [];
    ...
    // 7.5 BATCH LIMIT: Max 8 actions per response, force continuation if more
    const MAX_ACTIONS = 8;
    let actions = agentResult.actions;
    let done = agentResult.done;
    if (actions.length > MAX_ACTIONS) {
      console.log(`[chat] Truncating ${actions.length} actions to ${MAX_ACTIONS}, forcing continuation`);
      actions = actions.slice(0, MAX_ACTIONS);
      done = false; // Force continuation so agent can observe
    }
    ...
    // DISABLED: Memory feature turned off for now
    // if (done) {
    ...
    //     });
    //
    //   const allActions = [...priorActions, ...actions];
