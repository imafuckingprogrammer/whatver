/**
 * Returns the self-contained widget JavaScript that gets served as
 * /embed/:siteKey.js. The script derives its siteKey and server origin
 * from its own <script src> URL so no server-side templating is needed.
 *
 * Isolation strategy: Shadow DOM so zero CSS bleeds in or out.
 */
export function getWidgetScript(): string {
  ...
  function send() {
    ...
    var body = JSON.stringify({ siteKey: siteKey, visitorId: visitorId, message: text });
